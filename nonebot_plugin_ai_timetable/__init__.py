...
mytable = on_regex(r'^(小爱|我的|本周|下周)(课表)', priority=1, block=False)
newtable = on_command('导入课表', priority=1, block=False, aliases={'创建课表'})
...
def geturl(response):
    global real_url
    real_url = str(response.url) if re.match(
        real_url_re, response.url) else real_url
...
async def _(matcher: Matcher, bot: Bot, event: MessageEvent, key: str = ArgStr()):
    ...
    if re.match(base_url_re, url):  # 用户发送的链接匹配
        async with get_new_page(viewport={"width": 1000, "height": 1200}) as page:
            ...
            global real_url
            log_debug("response.url", real_url)
            async with httpx.AsyncClient() as r:
                res = await r.get(real_url)
        ...
        if uid in userdata:  # 这里保存的是小爱课程表分享的链接，就可以定时通过这里的链接更新本地课表，但是还不清楚链接是否会失效
            ...
            write_data()
            await newtable.finish("你已经导入过课表了，咱帮你更新课表数据了", at_sender=True)
        elif uid not in userdata:
            userdata.update({uid: url})
            write_data()
            await newtable.finish("成功导入课表！快发送课表帮助来查看功能吧！", at_sender=True)
    else:
